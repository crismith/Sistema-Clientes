<?php 
    include("php/conexion.php");
    $mensaje=false;
    if($_POST && $_POST["id_cliente"]>0) {

        $idcliente = $_POST['id_cliente'];
        $nombre = $_POST['nombre'];
        $empresa = $_POST['empresa'];
        $ubicacion = $_POST['ubicacion'];
        $ruc = $_POST['ruc'];
        $inicio = $_POST['inicio'];
        $vencimiento = $_POST['vencimiento'];
        $celular = $_POST['celular'];
        $email = $_POST['email'];
        
        $update="UPDATE clientes SET 
            nombre             =    '$nombre',
            nombre_negocio     =    '$empresa',
            ubicacion          =    '$ubicacion',
            ruc                =    '$ruc',
            fecha_inicio       =    '$inicio',
            fecha_vencimiento  =    '$vencimiento',
            celular            =    '$celular',
            email              =     '$email'   
            WHERE id_cliente = ".$idcliente;

        mysqli_query($conexion,$update);
        $mensaje = "se actualizo correctamente a nuestro cliente";

    }else if ($_GET && $_GET["id"]> 0) {
        $idcliente = $_GET["id"];
        $delete =  "DELETE FROM clientes WHERE id_cliente =".$idcliente;
        mysqli_query($conexion,$delete);
        $mensaje = "se elimino correctamente a nuestro cliente";
    } 
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <title>AdminClientes</title>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="esta es mi sistema web que me permite crear mis clientes">
    <meta name="author" content="cristian zeña mas">
    <!-- Bootstrap Core CSS -->
    <link rel="stylesheet" href="css/estilo.css">
    <link rel="stylesheet" href="css/bootstrap.css">
    <link rel="stylesheet" href="css/font-awesome.min.css">
</head>

<body>
    <!-- Navigation -->
    <nav class="navbar navbar-default navbar-custom navbar-fixed-top">
        <div class="container-fluid">
            <!-- Brand and toggle get grouped for better mobile display -->
            <div class="navbar-header page-scroll">
                <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#bs-example-navbar-collapse-1">
                    <span class="sr-only">Toggle navigation</span>
                    Menu <i class="fa fa-bars"></i>
                </button>
                <a class="navbar-brand" href="#!/">SisClient</a>
            </div>
            <!-- Collect the nav links, forms, and other content for toggling -->
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                <ul class="nav navbar-nav navbar-right">
                    <li>
                        <a href="index.php">Home</a>
                    </li>
                    <li>
                        <a href="registrar_cliente.php">Registrar</a>
                    </li>
                    <li>
                        <a href="lista-clientes.php">Clientes</a>
                    </li>
                </ul>
            </div>
            <!-- /.navbar-collapse -->
        </div>
        <!-- /.container -->
    </nav>

     <div class="cabezera">
        <div class="logo"></div>
    </div>


	<div class="container-fluid">
		<div class="container">
			<div class="row">
                <?php
                    if ($mensaje) { 
                        ?>
                            <div class ="alert alert-danger"><?php echo $mensaje ?></div>
                        <?php  
                    } 
                ?>
				<h1>Clientes</h1>
				<table class="table table-hover">
					<thead>
						<th>N°</th>
						<th>Cliente</th>
						<th>Empresa</th>
                        <th>Dirección</th>
						<th>RUC</th>
                        <th>Celular</th>
                        <th>F-Final</th>
						<th>DEL/UPD</th>
					</thead>
					<tbody>
						<?php  
                            $sql = "SELECT * FROM clientes";
                            $lista=mysqli_query($conexion,$sql);
                            $C=1;

                            if (mysqli_num_rows($lista)>0) {
                                    while($row=mysqli_fetch_assoc($lista)){
                                        echo "<tr >";
                                            echo "<td>".$C++."</td>";
                                            echo "<td>".$row["nombre"]."</td>";
                                            echo "<td>".$row["nombre_negocio"]."</td>";
                                            echo "<td>".$row["ubicacion"]."</td>";
                                            echo "<td>".$row["ruc"]."</td>";
                                            echo "<td>".$row["celular"]."</td>";
                                            echo "<td>".date($row["fecha_vencimiento"])."</td>";
                                            echo "<td>
                                                    <a href='clientes.php?id=".$row["id_cliente"]."'>
                                                        <i class='fa fa-trash-o'> Eliminar </i>
                                                    </a>&nbsp;    
                                                    <a href='actualizar.php?id=".$row["id_cliente"]."'>
                                                        <i class='fa fa-pencil'> Editar</i>
                                                    </a>
                                                </td>";
                                        echo "</tr>";
                                    }
                             }else{
                                echo "<h1>0 Registro¡¡¡</h1>";
                             }
                             mysqli_close($conexion);

                        ?>
					</tbody>
				</table>
			</div>
		</div>
	</div>

    <script src="js/jquery.js"> </script>
    <script src="js/bootstrap.js"> </script>
</body>

</html>